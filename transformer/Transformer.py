...
class CrossAttentionLayer(nn.Module):
    ...
    def forward_x(
        ...
        # query = self.norm1(query)
        pos = self.pe_layer(s_feature)
        # (batch_size, target_sequence, d_model)
        b, ch, h, w, = s_feature.shape
        query = query.repeat(1, 1, ch)
        pos_s = self.pe_layer(s_feature)
        pos_q = self.pe_layer(q_feature)

        # (batch_size, target_sequence, d_model)
        pos_s = pos_s.view(b, ch, -1).permute(0, 2, 1)
        pos_q = pos_q.view(b, ch, -1).permute(0, 2, 1)

        num_queries = query.shape[1]

        s_feature = s_feature.view(b, ch, -1).permute(0, 2, 1)
        # s_feature = self.norm1(s_feature)
        s_feature = self.with_pos_embed(s_feature, pos_s)

        q_feature = q_feature.view(b, ch, -1).permute(0, 2, 1) # [b, h*w, ch]
        # q_feature - self.norm2(q_feature)
        q_feature = self.with_pos_embed(q_feature, pos_q)
        
        key = torch.cat((s_feature, q_feature), dim=1) # [b, 2*h*w, ch]
        value = torch.cat((q_feature, s_feature), dim=1)
        # s_mask=[bsz, 1, h, w]
        s_mask = s_mask.view(b, -1).unsqueeze(1).repeat(1, num_queries, 1)  # [b, num_quereis, h*w]
        q_mask = q_mask.view(b, -1).unsqueeze(1).repeat(1, num_queries, 1)  # [b, num_quereis, h*w]
        attn_mask = torch.cat((s_mask, q_mask), dim=2) # [b, num_queries, 2*h*w]
        # 转换成bool tensor
        # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged
        attn_mask = attn_mask.unsqueeze(1).repeat(1, self.heads, 1, 1).view(b*self.heads, num_queries, 2*h*w)
        # 原mask为0的地方变为-1e10，原mask为1的地方变为0.0
        # attn_mask = attn_mask.masked_fill(attn_mask==0, float(1e-10)).masked_fill(attn_mask == 1, float(0.0))
        attn_mask = ~(attn_mask.bool())


        new_query = self.cross_attn(
            query=query,
            key=key,
            value=value,
            attn_mask=attn_mask
        )[0]
        new_query = torch.where(torch.isnan(new_query), torch.full_like(new_query, 0), new_query)
        # print(new_query[0]) nana
        return new_query
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    # support和query如何交互
    def input_process(self, s_features, q_features, s_masks, q_masks):
    ...
    def forward(self, s_features, q_features, s_masks, q_masks, mask_features):
        x, y, masks = self.input_process(s_features, q_features, s_masks, q_masks)
...
if __name__ == "__main__":
    ...
    predictor = MultiScaleMaskedTransformerDecoder([1024, 512, 256], hidden_dim=256,
            num_queries=100, nheads=8, dim_feedforward=2048, dec_layers=3, mask_dim=256, enforce_input_project=False, add_mask=False)
